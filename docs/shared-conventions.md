...
Engineering conventions shared between `mhdb-gen` and `mrdb-gen`. Imported into each project's CLAUDE.md via Claude Code's `@`-import syntax. Project-specific architecture and overrides live in each project's own CLAUDE.md / `docs/architecture/`.

Examples below use `mhdb-gen`'s `Event*` vocabulary as illustrative; the rules are domain-agnostic and apply identically when read with `Record*` / `Artist*` substituted.
