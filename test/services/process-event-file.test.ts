...
`
  ...
  describe('when an error occurs anywhere in the pipeline', () => {
    it('wraps the cause in an EventFileError carrying the path', () => {
      ...
      /** Path to the illustration fixture; the markdown read throws before this is touched. */
      const illustrationPath = join(tmpDirPath, 'missing.png')

      writeFileSync(illustrationPath, 'illustration-bytes')

      assert.throws(
        () => processEventFile(db, missingPath, illustrationPath),
