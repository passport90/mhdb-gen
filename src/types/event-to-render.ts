 ...
 * Produced by the render service by combining an `EventListing` (slot + id + slug + updatedAt)
...
interface EventToRender extends EventBody, EventSlot {
  ...
  id: number
  /** URL slug derived from `title` via `slugify`; carried over from the upstream `EventListing`. */
  slug: string
